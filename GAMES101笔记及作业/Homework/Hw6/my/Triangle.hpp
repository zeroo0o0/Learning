...
inline Intersection Triangle::getIntersection(Ray ray)
{
	...
	//这一块对HW5 Triangle.hpp的rayTriangleIntersect函数稍作修改
	//并且按照新框架更新相应相交信息的格式即可
	 Vector3f e1=v1-v0;
	 Vector3f e2=v2-v0;
	 Vector3f s0=ray.origin-v0; //orig替换为ray.origin
	 Vector3f s1=crossProduct(ray.direction ,e2); //dir 换成ray.direction 
	...
	if(tnear>0&&u>=0&&v>=0&&(1-u-v)>=0)  //u+v<=1也是一样的
	{
		//按照新框架更新相应相交信息
		inter.happened =true;
		inter.coords =Vector3f(1-u-v,u,v);//重心坐标
		inter.normal=normal;
		inter.m=m;
		inter.obj=this;
		inter.distance =tnear;//距离
		// return inter;  //相交  在最后return即可
	} 
