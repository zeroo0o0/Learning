...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
	...
	for(int i=0;i<3;i++) {
		const Vector3f &P=t.v[i]; //因为t.v是const
		...
		for(int x=aabb_minX; x< aabb_maxX;x++) {
		 ...
		 for(int x = (int)aabb_minX; x < aabb_maxX; x++) {
			...
			// If so, use the following code to get the interpolated z value.
	                ...
	                set_pixel(Vector3f(x,y,z_interpolated),t.getColor()); //Color
    			}
    		}//TODP_END
    	}
    }
...
void rst::rasterizer::mix_pixel(const Vector3f& point,const Vector3f& color) //声明是const这里也得是
{
	auto index = (height-1-point.y())*width+point.x(); //找出下标
